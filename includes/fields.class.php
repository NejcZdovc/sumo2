...
{	
	...
	public function input($min, $max, $fieldId, $required="0", $value=null, $dynamic=false, $form=null) {
		global $db;
		if($required=="1") {
            if($form!=null) {
                $this->javascript.="var element=document.".$form.".".$fieldId.";";
            } else {
	...
	public function email($fieldId, $value=null, $required="0", $dynamic=false, $form=null) {
		global $db;
        if($form!=null) {
             $this->javascript.="var element=document.".$form.".".$fieldId."; var reg = /^([a-zA-Z0-9\u00A1-\uFFFF]+([\.+_-][a-zA-Z0-9\u00A1-\uFFFF]+)*)@(([a-zA-Z0-9]+((\.|[-]{1,2})[a-zA-Z0-9]+)*)\.[a-zA-Z]{2,6})$/;";
        } else {
	...
	public function password($min, $fieldId, $required, $value=null, $dynamic=false, $form=null) {
		global $db;
		if($required=="1") {
             if($form!=null) {
                $this->javascript.="var element=document.".$form.".".$fieldId.";";
            ...
            $this->javascript.="if (element)  {if(element.value.length<".$min.") {ok=false;element.className=element.className.replace('error','');element.className+='error';} else {element.className=element.className.replace('error','');}}";
		}
		if($dynamic && $db->is($fieldId))
	...
	public function check($fieldId, $options, $required, $position, $value=null, $dynamic=false, $form=null) {
		global $db;
		...
		if($required=="1") {
            if($form!=null) {
                $this->javascript.="var element=document.forms['".$form."'].elements['".$fieldId."[]'];";
            } else {
        ...
        if(count($options)==1 && $form!=null) {
            $code.='<input type="checkbox" disabled="disabled" style="display:none !important;" name="'.$fieldId.'[]" class="'.$fieldId.'" id="'.$fieldId.'_'.$stevilo.'" value="-1" />';
        }
		return $code;
	}
	
	public function text($min, $max, $fieldId, $required="0", $value=null, $dynamic=false, $form=null) {
		global $db;
		if($required=="1") {
            if($form!=null) {
                $this->javascript.="var element=document.".$form.".".$fieldId.";";
