<?php 
	...
	if(isset($_POST['type'])) {
		...
		if($_POST['type'] == 'articleR') {
			...
			echo "ok";
			exit;
		} else if($_POST['type'] == 'userR') {
			$id=$crypt->decrypt($db->filter('id'));
			$db->query('UPDATE cms_user SET status="N" WHERE ID='.$id.'');
			echo "ok";
			exit;
		} else if($_POST['type'] == 'userGR') {
			$id=$crypt->decrypt($db->filter('id'));
		...
		//Delete
		else if($_POST['type'] == 'articleD') {
			$id=$crypt->decrypt($db->filter('id'));
			$db->query('DELETE FROM cms_article WHERE ID='.$id.'');
			echo "ok";
			exit;
		} else if($_POST['type'] == 'articleGD') {
			$id=$crypt->decrypt($db->filter('id'));
			$articleq = $db->query('SELECT ID,category FROM cms_article WHERE category='.$id.'');
			while($articleF=$db->fetch($articleq)) {
		...
		//DeleteAll
		else if($_POST['type'] == 'DD_#article') {
		...
		else if($_POST['type'] == 'DD_#articleG') {
			$idT=explode('!!##!!!', $db->filter('id'));
			foreach($idT as $ids) {
				if(strlen($ids)>3) {
					$id=$crypt->decrypt($ids);
