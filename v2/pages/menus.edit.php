...
<form action="" name="d_menus_edit_m" method="post" class="form2">
    ...
    <tr>
        ...
        <div class="title_form_big"><?=$lang->NAME?>:</div><div class="title_form_small"><?=$lang->MENU_21?></div>
        </td>
        <td class="right_td">
        <input type="hidden" id="id" value="<?= $db->filter('id')?>" />
        <input name="name" id="name" value="<?=$results['title']?>" type="text" maxlength="50" class="input" />
    ...
    <tr>
    	<td colspan="2" class="right_td" style="padding:5px;">
			<textarea  id="contentd" class="input-area" name="content" rows="10" cols="50"><?=$results['description']?></textarea>
        </td>
