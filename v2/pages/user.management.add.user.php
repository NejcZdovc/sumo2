...
<form action="" name="d_user_add_user" method="post" class="form2">
    ...
    <tr>
        <td class="left_td" valign="top">
        <div class="title_form_big"><?=$lang->USER_ADDU_P_1?>:</div><div class="title_form_small"><?=$lang->USER_ADDU_P_2?></div>
        </td>
        <td class="right_td">
    ...
    <tr>
        ...
        <div class="flt-right password"><div><div id="password_indicator_d_user_add_user"><div style="display:block;background-image:url(images/css_sprite.png);background-position:-1109px -1634px;width:230px;height:19px;"></div></div></div><div id="password_strength_d_user_add_user" class="password-title"><?=$lang->USER_ADDU_P_7?></div></div>
        </td>
    </tr>
    <tr><td height="10px" width="100%" colspan="2"></td></tr>
     <tr>
	...
	while($result = $db->fetch($fieldQuery)) {
		switch($result['type']) {
			case 1:
				...
				<td class="right_td">
				<input name="<?=$result['name']?>" id="<?=$result['fieldId']?>#<?=$result['type']?>#<?=$result['required']?>#<?=$result['labelName']?>#<?=$result['min']?>" class="input extra" value="" type="text" maxlength="<?=$result['max']?>"/>
				</td>
			    </tr>
				<?php
				break;
				...
				?>
					...
					<tr>
					    	<td colspan="2" class="right_td" style="padding:5px;">
								<textarea  id="<?=$result['fieldId']?>#<?=$result['type']?>#<?=$result['required']?>#<?=$result['labelName']?>#<?=$result['min']?>#<?=$result['max']?>" name="<?=$result['name']?>" rows="10" class="input-area extra" cols="54"></textarea>
						</td>
			...
			case 3:
				...
				<td class="right_td">
				<input name="<?=$result['name']?>" id="<?=$result['fieldId']?>#<?=$result['type']?>#<?=$result['required']?>#<?=$result['labelName']?>#<?=$result['min']?>" class="input extra sumo2-dcsibfvs" value="" type="text" maxlength="<?=$result['max']?>" readonly="readonly"/>
				</td>
			    </tr>
				<?php
				break;
