...
?>
<form id="">
<input id="selected_lang_menu_id" value="<?=$selected_lang_menus?>" type="hidden"/>
</form>
...
<table cellpadding="0" cellspacing="1" border="0" class="table1 table2" id="viewgroups" width="99%">
    ...
    <th></th>
        ...
        <? if($user->getAuth('FAV_SITE_5') == 2 || $user->getAuth('FAV_SITE_5') == 4 || $user->getAuth('FAV_SITE_5') == 5)
	...
	<?php 
		...
		while($result = $db->fetch($query)) {
			...
			$rows = $db->rows($items_query);
				...
				?>
                	<td width="17px" style="cursor:pointer;" onclick="sumo2.menu.Dialog_m('<?= $crypt->encrypt($result['ID']); ?>')"><? if($result['s_default']==1) echo '<div style="height:16px;width:16px;background:url(/v2/images/css_sprite.png); background-position:-669px -1709px; cursor:pointer;"></div>';?></td>
                    ...
                    <? if($user->getAuth('FAV_SITE_5') == 2 || $user->getAuth('FAV_SITE_5') == 4 || $user->getAuth('FAV_SITE_5') == 5) {?>
					<td width="100px">
						<div title="<?=$lang->MENU_13?>" class="<?php echo $result['enabled']?"enable":"disable"; ?> sumo2-tooltip" onclick="sumo2.menu.Status('<?php echo $crypt->encrypt($result['ID']); ?>')"></div>
                        <div title="<?=$lang->MENU_14?>" class="add sumo2-tooltip" onclick="sumo2.dialog.NewDialog('<?=$dialog?>','id=<?= $crypt->encrypt($result['ID']); ?>$!$items=<?= $crypt->encrypt($items) ?>$!$choos=-666$!$current=<?=$crypt->encrypt($selected_lang_menus)?>');"></div>
						<div title="<?=$lang->MENU_15?>" class="edit sumo2-tooltip" onclick="sumo2.dialog.NewDialog('d_menus_edit_m','id=<?= $crypt->encrypt($result['ID']); ?>');"></div>
						<div title="<?=$lang->MENU_16?>" class="delete sumo2-tooltip" onclick="sumo2.menu.DeleteMenu('<?php echo $crypt->encrypt($result['ID']); ?>')"></div>
					</td>
                    <? } if($user->translate_state=="ON") { ?>
                    <td width="150px" style="text-align:center;">
                    <select id="lang_trans">
                    <option value="0" >--- <?=$lang->MENU_17?> ---</option>
                   <?
